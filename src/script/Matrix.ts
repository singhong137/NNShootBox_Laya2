...
export class Matrix {
    ...
    constructor(rows: number, cols: number, data = []) {
        this._rows = rows;
        this._cols = cols;
        this._data = data;
    ...
    // add two matrices
    static add(m0: Matrix, m1: Matrix) {
    ...
    static checkDimensions(m0: Matrix, m1: Matrix) {
    ...
    static convertFromArray(arr: number[]): Matrix {
        return new Matrix(1, arr.length, [arr]);
    }

    // dot product of two matrices
    static dot(m0: Matrix, m1: Matrix): Matrix {
        if (m0.cols != m1.rows) {
    ...
    static map(m0: Matrix, mFunction: Function): Matrix {
        let m = new Matrix(m0.rows, m0.cols);
        for (let i = 0; i < m.rows; i++) {
            for (let j = 0; j < m.cols; j++) {
    ...
    static subtract(m0: Matrix, m1: Matrix): Matrix {
        Matrix.checkDimensions(m0, m1);
        let m = new Matrix(m0.rows, m1.cols);
        for (let i = 0; i < m.rows; i++) {
            for (let j = 0; j < m.cols; j++) {
    ...
    static transpose(m0: Matrix): Matrix {
